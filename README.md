...
deno test -A --no-check packages/kernel packages/sdk-server packages/mcp-server
...
[Deno Sandbox](https://deno.com/deploy/kernel) is a cloud service (beta, launched Feb 2026) running lightweight Linux microVMs on Deno Deploy. Like E2B, it provides full Linux environments with any language support. Unlike E2B, it's cloud-only with no self-hosting option.
...
This project was written entirely by Claude (Anthropic's AI assistant) as an experiment in AI-driven software engineering. Every line of code — the Rust shell parser, the TypeScript kernel, the Python SDK, the BusyBox/WASM toolchain integration, and the build tooling — was generated by Claude across a series of collaborative sessions with a human directing the design.
