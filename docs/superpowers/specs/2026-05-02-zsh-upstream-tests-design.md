...
Upstream zsh is a good integration target because its own test harness is
written in zsh and runs many small process, redirection, substitution, and
builtin cases. If zsh can run its own tests in Codepod, the fork work is being
tested against a substantially more realistic workload.
...
3. Add a richer BusyBox build or BusyBox variant that includes the external
   commands needed by the zsh test suite.
4. Make a serious attempt to build GNU make for Codepod and use it to drive
   zsh's upstream `make check` path inside the sandbox.
5. Run upstream zsh tests inside Codepod, using `TESTNUM` category slices for
   triage and a full-suite target for final acceptance.
6. Treat failures as platform findings: classify each as zsh port issue,
   BusyBox/userland gap, GNU make gap, guest-compat syscall gap, kernel process
   bug, or unsupported terminal/module feature.
7. Keep all source inputs pinned and reproducible.
...
2. The runtime starts zsh through the normal sandbox process path, not a custom
   host escape.
3. The sandbox VFS contains the zsh build tree, `Src/zsh`, `Test/*.ztst`,
   `Test/runtests.zsh`, and the external tools needed by the tests.
4. GNU make is used to invoke zsh's `make check` path unless the bounded make
   investigation reaches the explicit side-quest gate described below.
5. If GNU make becomes a disproportionate side quest, the implementation must
   stop and present evidence before switching to a direct `runtests.zsh` driver.
6. A direct driver, if approved, must set the same essential environment as
...
7. Test results are persisted as machine-readable output and a human summary:
...
The existing BusyBox port is too narrow for zsh's test suite. This work needs a
test-support BusyBox build that includes at least:
...
3. Mount or copy the zsh build/test tree into the sandbox at `/work/zsh`.
4. Run category slices with explicit `TESTNUM` values.
5. Capture stdout/stderr, exit status, timeout, and per-test summary lines.
6. Emit JSON summary to `packages/c-ports/zsh/build/test-results/*.json`.
...
`zsh -f Test/runtests.zsh` with the upstream-equivalent env and records that
mode in the result JSON.
...
1. **Build smoke:** `zsh -fc 'print ok'`
2. **Process smoke:** `zsh -fc 'print parent=$$; (print child=$$); print done'`
3. **Pipeline smoke:** `zsh -fc 'print hello | cat'`
4. **Exec smoke:** `zsh -fc 'command true; command false; print $?'`
5. **Harness smoke:** one simple `.ztst` file through `runtests.zsh`
6. **Category A:** basic command parsing and execution
7. **Category B:** builtins
8. **Category C:** special shell command syntax
9. **Category D:** substitution
10. **Category E/K/Z:** options, ksh features, separate systems/contrib
11. **Categories V/W/X/Y:** modules, interactive constructs, ZLE, completion
12. **Full suite:** upstream `make check`
...
packages/c-ports/busybox/
  busybox.config or busybox-tests.config
  manifest.json or busybox-tests.manifest.json
...
2. **Terminal behavior may block interactive categories.** The non-interactive
  ...
  model.
3. **Make may expose more process holes than zsh itself.** That is useful if the
   holes are core platform behavior; otherwise use the side-quest gate.
4. **Asyncify overhead may be high.** zsh is large and continuation-enabled.
   The test runner should use generous but finite timeouts and report timeouts
   distinctly from semantic failures.
5. **BusyBox breadth can mask missing standalone tools.** The test harness must
...
2. Add BusyBox test applets before running zsh tests.
3. Attempt GNU make before adding a direct zsh harness fallback.
4. Add small canaries for any platform behavior needed by zsh failures.
5. Keep commits slice-sized: source pins, BusyBox expansion, make port, zsh port,
   runner, then platform fixes.
