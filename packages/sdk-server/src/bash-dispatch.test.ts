...
Deno.test('runCommand drives PID 1 through the bash protocol', async () => {
  const sb = await Sandbox.create({
    ...
    bootImports: bashBootImports as unknown as Parameters<typeof Sandbox.create>[0]["bootImports"],
    runCommandHandler: makeRunCommandHandler(),
  } as unknown as Parameters<typeof Sandbox.create>[0] & Record<string, unknown>);
  try {
