...
describe('history builtin', () => {
  ...
  it('history list returns executed commands', async () => {
    sandbox = await Sandbox.create({ wasmDir: WASM_DIR, shellWasmPath: SHELL_WASM, adapter: new NodeAdapter(), shellBackend: 'typescript' });
...
describe('cross-feature integration', () => {
  ...
  it('file data survives export/import', async () => {
    ...
    const sandbox2 = await Sandbox.create({ wasmDir: WASM_DIR, shellWasmPath: SHELL_WASM, adapter: new NodeAdapter(), shellBackend: 'typescript' });
    try {
